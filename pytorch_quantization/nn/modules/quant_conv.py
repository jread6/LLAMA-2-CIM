...
from pytorch_quantization.cim import cim_conv
...
# sys.path.append("/usr/scratch1/james/TensorRT/tools/pytorch-quantization/pytorch_quantization/cim")
# from args import CIM_ARGS
...
class _QuantConvNd(torch.nn.modules.conv._ConvNd, _utils.QuantMixin):
    ...
    def _quant(self, input):
        ...
        """
        if self.cim_args is not None and self.cim_args.inference == True:
            # return scale for input and weight
            quant_input, scale_input = self._input_quantizer(input, self.cim_args.inference)
            quant_weight, scale_weight = self._weight_quantizer(self.weight, self.cim_args.inference)

            return (quant_input, scale_input, quant_weight, scale_weight)
...
class QuantConv2d(_QuantConvNd):
    ...
    def __init__(self,
        ...
        cim_args = self.__class__.cim_args
    ...
    def forward(self, input):
        ...
        if self.cim_args is not None and self.cim_args.inference == True:
            quant_input, scale_input, quant_weight, scale_weight = self._quant(input)
        ...
        if self.padding_mode == 'circular':
            ...
            if self.cim_args is not None and self.cim_args.inference == True:
                output = cim_conv.cim_conv2d(self.cim_args, F.pad(quant_input, expanded_padding, mode='circular'),
                                               scale_input, quant_weight, scale_weight, self.bias, 
            ...
            if self.cim_args is not None and self.cim_args.inference == True:                
                output = cim_conv.cim_conv2d(self.cim_args, quant_input, scale_input, quant_weight, scale_weight,
        ...
        # if not torch.allclose(output_correct, output, atol=1e-3):
        #     print("Conv2d output is not close to the correct output")
        #     # print minimum and maximum of output_correct and output
        #     print("output_correct min: ", output_correct.min())
        #     print("output_correct max: ", output_correct.max())
