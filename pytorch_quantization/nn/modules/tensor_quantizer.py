...
class TensorQuantizer(nn.Module):
    ...
    def _quant_forward(self, inputs, cim=False):
        ...
        if self._fake_quant:
            if not TensorQuantizer.use_fb_fake_quant:
                if cim:
                    # save scale from fake quant
                    outputs, scale = fake_tensor_quant(inputs, amax, self._num_bits, self._unsigned, self._narrow_range, cim)
    ...
    def forward(self, inputs, cim=False):
        ...
        if self._if_quant:
            if cim:
                outputs, scales = self._quant_forward(inputs, cim)
                return outputs, scales
            else:
                outputs = self._quant_forward(inputs, cim)
