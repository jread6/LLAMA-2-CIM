...
from pytorch_quantization.cim import cim_linear
...
class QuantLinear(nn.Linear, _utils.QuantMixin):
    ...
    def forward(self, input):
        if self.cim_args is not None and self.cim_args.inference == True:
            ...
            output = cim_linear.cim_linear(self.cim_args, quant_input, scale_input, quant_weight, 
                                           scale_weight, self.bias)
            
            # output_correct = F.linear(quant_input, quant_weight, bias=self.bias)
