...
def simulate_array(args, input2d, weight2d):
    ...
    for i in range(args.input_precision):
        ...
        for part in range(num_partitions):  
            ...
            # scale by reduction in ADC precision
            # scale = math.ceil(math.log(args.sub_array[0], 2)) - args.adc_precision
            # out = out << scale
...
def ADC_output(args, inputs, weights):
    ...
    if args.v_noise > 0:
        ...
        # print("min: ", torch.min(noise))
        # print("max: ", torch.max(noise)) 
...
def calc_vref(args):
    ...
    # args.v_ref *= args.open_rows // 2**args.adc_precision
...
def sense_voltage(args, BL_voltages):
    ...
    # ADC_output *= args.open_rows // 2**args.adc_precision
